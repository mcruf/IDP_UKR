...
cols <- c('gray95',
          ...
          "#f9b339",
          "#df5940",
          "#9c100f") #From 0 - 7 images



dobs2b$nobs <- as.factor(dobs2b$nobs)
...
dobs2b %>%  
  ...
  geom_tile(colour = "white", linewidth = 0.5) +
  geom_text(col='white') +
  scale_fill_manual(values = cols, na.value="gray95") +
  #scale_fill_gradientn(colours = cols, limits=c(1, 7), na.value="gray95") + 
  #scale_fill_gradientn(colours = c("gray95", "blue", "red"), values = c(0,0.2,0.8)) +
  #scale_fill_gradient(low="blue", high="red", limits=c(1, 7),na.value="white") +
  theme_bw() +
  ...
  theme( axis.text.x = element_text(angle = 90,size = 13, vjust = 0.5, hjust = 1, color = c(rep('#2B9EB3', dim(west)[1]), rep('#EFAB1B', dim(central)[1]), rep('#A28377', dim(east)[1])), face='bold'),
         axis.text.y = element_text(size = 13, face='bold'),
         axis.title.x = element_text(size = 14, face='bold', margin = unit(c(3, 0, 0, 0), "mm")),
         axis.title.y = element_text(size = 14, face='bold', margin = unit(c(0, 3, 0, 0), "mm")),
         axis.ticks = element_line(colour = "gray40"),
         legend.title.align=0.5,
