...
#                on a several spatial granularity                    #
#                                                                    #
...
if(DINPUT == "Count"){
  ...
  for(i in seq_along(spgrid_list)){
    ...
    colnames(worldpop_grid[[i]]) <- c("grid_id", "Total_Dpop")
  }
}
...
setdiff(unique(prop$City), unique(popcar$City)) #Nope, we're good to go

## Define the threhsold (here, 50%) and list images below the threshold
prop$Classification <- ifelse(prop$Proportion < 0.5, "Below", "Above") # Below = below 50%, Above = above 50%
prop_below <- filter(prop, Classification == "Below") #Keep images below threshold
...
if(CARS == "original"){
  ...
  if(THRESHOLD == 'TH_15'){
    ...
    OUTDIR <- "Imagery_EDA/WorldPop_vs_Ncars/Confidence_Threshold/TH45/carclass_18/osm_filtered/"
  }
}
...
rm(list = setdiff(ls(), c('popcar_aggr_grid','spgrid_aoi', 'spgrid_img', 'OUT')))
