...
## Data rearrangement & additional calculations
tmp <- dfg2 %>%
...
## Split data by Baseline and everything else
tmp1 <- tmp %>% filter(Method == 'Base') # Baseline
tmp2 <- tmp %>% filter(!(Method %in% 'Base')) #Covid & war period


## Plot 1: IDP predictions
p1 <- tmp2 %>%
...
# OUTFILE <- "~/Downloads/IDP_plot.jpg"
# 
